...
class TopOneRight extends Component {
    constructor(props) {
        super(props);
        this.state = {
            mycharts1: "",
    ...
    render() {
        return (
            <div>
                <div id="TopOneRight" ref='TopOneRight' style={{ width:340, height: 200 }}></div>
            </div>
        );
    }
    ...
    componentDidMount() {
        let that = this;
        this.mycharts1 = echarts.init(document.getElementById("TopOneRight"));
        this.option = {
            color: ['#3398DB'],
            title: {
                x:'center',
    ...
    resizeContainer(){
        ...
        $("#TopOneRight").width (top_left_width);
        $("#TopOneRight").height (top_left_height);
    }

    handlebtnClick(e) {
        //let btime = Date.parse(this.begintime.value);
        //let etime = Date.parse(this.endtime.value);
